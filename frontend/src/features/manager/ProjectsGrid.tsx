...
const PRIORITY_COLOR: Record<string, string> = {
  LOW: '#00D26A', MEDIUM: '#FFC247', HIGH: '#EF4444', URGENT: '#8B1532',
};
...
export function ProjectsGrid({ initialData }: Props) {
  ...
  const rolePrefix = pathname.startsWith('/admin') ? '/admin' : '/manager';
            ...
            <div key={p.id} className="glass-card p-5 flex flex-col gap-4 group">
                    ...
                    {menu === p.id && (
                      <>
                        ...
                        <div className="absolute right-0 top-full mt-1 w-44 z-20 bg-[#0B1224] border border-white/[0.10] rounded-xl shadow-xl overflow-hidden">
                          <Link href={`${rolePrefix}/projects/${p.id}`}
              ...
              <div>
                ...
                <div className="h-1.5 bg-white/[0.08] rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-gradient-to-r from-[#730E27] to-[#8B1532] transition-all"
