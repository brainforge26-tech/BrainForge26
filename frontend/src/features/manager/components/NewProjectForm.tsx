...
import { createProjectAction, fetchClients } from '@/features/manager/manager.actions';
import type { ActionState, Client } from '@/features/manager/manager.actions';
...
export function NewProjectForm() {
  ...
  useEffect(() => {
    if (state.success) {
      ...
      router.push(isAdmin ? '/admin/projects' : '/manager/projects');
    }
  }, [state.success, state, router, isAdmin]);

  return (
    <div className="animate-fade-up max-w-2xl">
      <PageHeader
        title="New Project"
        description="Create a new client project with custom images and details."
        action={
          <button
            onClick={() => router.back()}
      ...
      <Card variant="default" padding="lg">
        <CardContent>
          <form action={formAction} className="space-y-5">
            ...
            {/* Project Image URL */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-[#AAB3C5] flex items-center gap-2">
                <ImageIcon className="w-4 h-4 text-[#8B1532]" /> Project Picture / Cover Image URL
              </label>
              <input
                name="imageUrl"
              ...
              />
              <p className="text-xs text-[#7A8499]">Paste image link to display in project showcases and gallery</p>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <select name="projectType" className="input-field">
                  ...
                  {['Web Application','Mobile App','E-Commerce','API / Backend','Landing Page','SaaS','Enterprise System'].map(t => (
            ...
            {/* Client ID */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-[#AAB3C5]">Client</label>
              <select name="clientId" className="input-field">
                <option value="">Select client (Optional - auto-assigned if empty)…</option>
                {clients.map(c => (
            ...
            {/* Manager notes */}
            <div className="space-y-1.5">
              <label className="text-sm font-medium text-[#AAB3C5]">Manager Notes <span className="text-[#7A8499] text-xs">(internal context)</span></label>
              <textarea name="managerNotes" rows={2} placeholder="Internal notes…" className="input-field resize-none py-3" />
            </div>
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => router.back()} className="btn-secondary px-6 py-2.5 rounded-full text-sm">
                Cancel
              </button>
            </div>
          </form>
