...
import { MapPin, Clock, DollarSign, ArrowRight, CheckCircle2, Sparkles } from 'lucide-react';
import { FormattedText } from '@/components/ui/FormattedText';
import { JobApplicationModal } from '@/components/jobs/JobApplicationModal';

export function CareersClientList({ jobs }: { jobs: any[] }) {
  const [selectedJob, setSelectedJob] = React.useState<{ id?: string; title?: string } | null>(null);
  const [modalOpen, setModalOpen] = React.useState(false);

  const handleOpenModal = (job?: any) => {
    setSelectedJob(job ? { id: job.id, title: job.title } : null);
    setModalOpen(true);
  };
