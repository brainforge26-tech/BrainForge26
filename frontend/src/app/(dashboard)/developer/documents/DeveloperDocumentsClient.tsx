...
export function DeveloperDocumentsClient({ resumeUrl: initialResumeUrl }: { resumeUrl: string }) {
        ...
        <CardContent className="space-y-4 mt-2">
          {initialResumeUrl ? (
            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-white truncate max-w-[200px] sm:max-w-md">{initialResumeUrl.split('/').pop() || 'Current Resume'}</p>
                  <p className="text-xs text-[#7A8499]">Synced</p>
                </div>
              </div>
              <div className="flex gap-2">
                <a href={initialResumeUrl} target="_blank" rel="noopener noreferrer"
            ...
            <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] text-sm text-[#7A8499]">
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-3">
              ...
              />
              <button onClick={handleUpdateResume} disabled={isPending || resumeUrl === initialResumeUrl}
                className="btn-primary px-4 py-2.5 rounded-full text-sm flex items-center gap-2 shrink-0 disabled:opacity-50">
      ...
      <Card variant="default" padding="none" className="overflow-hidden">
        <CardHeader className="px-6 pt-6 pb-4 border-b border-white/[0.06]">
          <div className="flex items-center justify-between">
            <CardTitle>Other Documents (Samples)</CardTitle>
