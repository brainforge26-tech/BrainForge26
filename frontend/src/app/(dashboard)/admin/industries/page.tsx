...
export default function AdminIndustriesPage() {
  ...
  const loadIndustries = async () => {
    try {
      setLoading(true);
      const { data } = await apiClient.get('/portfolio/industries');
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        await apiClient.put(`/portfolio/industries/${editingId}`, payload);
      } else {
        await apiClient.post('/portfolio/industries', payload);
      }
      setModalOpen(false);
      loadIndustries();
    } catch (err) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await apiClient.delete(`/portfolio/industries/${id}`);
      loadIndustries();
    } catch (err) {
  ...
  return (
    <div className="space-y-6 text-slate-100 max-w-4xl">
      <div className="flex items-center justify-between bg-[#090D16] p-6 rounded-2xl border border-white/[0.08]">
        <div>
          <h1 className="text-2xl font-extrabold text-white flex items-center gap-2.5">
            ...
            Industry Verticals CMS
          </h1>
          <p className="text-xs text-slate-400 mt-1">Manage industry sectors served (Fintech, Healthcare, Logistics, etc.).</p>
        </div>
        <button
      ...
      <div className="space-y-4">
        {industries.map(ind => (
          <div key={ind.id} className="p-6 rounded-2xl bg-[#0B1224] border border-white/[0.08] flex items-center justify-between shadow-xl">
            <div>
              <h3 className="font-bold text-white text-base mb-1">{ind.name}</h3>
              <p className="text-xs text-slate-400">{ind.description}</p>
            </div>
            <div className="flex gap-2">
              <button onClick={() => openEditModal(ind)} className="p-2 rounded-lg bg-blue-600/20 text-cyan-300 hover:bg-blue-600 hover:text-white">
                <Edit2 className="w-4 h-4" />
              </button>
              <button onClick={() => handleDelete(ind.id)} className="p-2 rounded-lg bg-rose-600/20 text-rose-300 hover:bg-rose-600 hover:text-white">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="w-full max-w-md bg-[#0E1526] border border-white/[0.1] rounded-3xl p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/[0.08] pb-3">
              <h3 className="text-lg font-bold text-white">{editingId ? 'Edit Industry' : 'Add Industry'}</h3>
              <button onClick={() => setModalOpen(false)} className="text-slate-400 hover:text-white"><X className="w-5 h-5" /></button>
            </div>

            <form onSubmit={handleSave} className="space-y-4 text-sm">
              <div>
                <label className="block text-xs font-bold uppercase text-slate-300 mb-1">Industry Name *</label>
              ...
              <div className="flex justify-end gap-3 pt-3 border-t border-white/[0.08]">
                ...
                <button type="submit" className="px-5 py-2 rounded-xl bg-blue-600 text-white font-bold">Save Industry</button>
              </div>
            </form>
          </div>
        </div>
