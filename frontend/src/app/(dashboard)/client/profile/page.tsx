...
const SAMPLE = {
  ...
  companyDescription: 'TechCorp is a fast-growing e-commerce company building the future of online retail. We serve 500k+ customers worldwide.',
...
export default function ClientProfilePage() {
  ...
  useEffect(() => {
    if (state.success) { toast.success(state.message); setEditing(false); }
  }, [state.success, state]);
        ...
        <CardHeader>
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-4">
              <Avatar name={SAMPLE.companyName} size="xl" />
              <div>
                <h2 className="text-xl font-bold text-white">{SAMPLE.companyName}</h2>
                <p className="text-sm text-[#7A8499] mt-0.5">{SAMPLE.contactPerson}</p>
                <p className="text-xs text-[#7A8499] mt-1">
                  Member since {new Date(SAMPLE.memberSince).toLocaleDateString('en-US',{month:'long',year:'numeric'})}
                </p>
              </div>
            </div>
        ...
        <CardContent>
          {!editing ? (
            <div className="space-y-4 mt-4">
              {[
                { icon: User,      label: 'Contact Person', value: SAMPLE.contactPerson },
                { icon: Building2, label: 'Company',        value: SAMPLE.companyName  },
                { icon: Phone,     label: 'Phone',          value: SAMPLE.phone        },
                { icon: Globe,     label: 'Website',        value: SAMPLE.website      },
                { icon: MapPin,    label: 'Address',        value: SAMPLE.address      },
            ...
            <form action={formAction} className="space-y-4 mt-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  ...
                  <input name="companyName" defaultValue={SAMPLE.companyName} className="input-field" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-[#AAB3C5]">Contact Person</label>
                  <input name="contactPerson" defaultValue={SAMPLE.contactPerson} className="input-field" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-[#AAB3C5]">Phone</label>
                  <input name="phone" type="tel" defaultValue={SAMPLE.phone} className="input-field" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-sm font-medium text-[#AAB3C5]">Website</label>
                  <input name="website" type="url" defaultValue={SAMPLE.website} className="input-field" />
                </div>
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-[#AAB3C5]">Address</label>
                <input name="address" defaultValue={SAMPLE.address} className="input-field" />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium text-[#AAB3C5]">Company Description</label>
                <textarea name="companyDescription" rows={3} defaultValue={SAMPLE.companyDescription}
